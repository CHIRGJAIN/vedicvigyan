...
router.get('/', (req, res) => {
    ...
    let events = Array.from(store_1.db.events.values()).filter(e => e.isActive);
    // Filter by status
    if (status) {
    ...
    if (search) {
        ...
        events = events.filter(e => e.name.toLowerCase().includes(searchTerm) ||
...
router.get('/:id', (req, res) => {
    const event = store_1.db.events.get(req.params.id);
...
router.post('/', auth_1.requireAuth, (req, res) => {
    ...
    const { name, description, date, time, location, maxAttendees, price, category, speaker, thumbnail } = req.body;
    ...
    store_1.db.events.set(id, event);
...
router.put('/:id', auth_1.requireAuth, (req, res) => {
    ...
    store_1.db.events.set(event.id, updatedEvent);
...
router.delete('/:id', auth_1.requireAuth, (req, res) => {
    ...
    store_1.db.events.set(event.id, event);
...
router.post('/:id/register', auth_1.requireAuth, (req, res) => {
    ...
    const existingRegistration = Array.from(store_1.db.eventRegistrations.values())
    ...
    store_1.db.eventRegistrations.set(registrationId, registration);
...
router.get('/user', auth_1.requireAuth, (req, res) => {
    const userRegistrations = Array.from(store_1.db.eventRegistrations.values())
...
router.get('/:id/attendees', auth_1.requireAuth, (req, res) => {
    if (req.user?.role !== 'admin') {
        return res.status(403).json({ message: 'Forbidden' });
    }
    const event = store_1.db.events.get(req.params.id);
